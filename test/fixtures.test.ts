...
test("fixture regeneration is deterministic (regenerate → byte-identical to committed)", () => {
  // PHASE_REVIEW_1 SHOULD-FIX 1: actually regenerate into a temp dir and
  // byte-compare every output against the committed fixtures.
  const tmp = mkdtempSync(join(tmpdir(), "meridian-fixtures-"));
  try {
    execFileSync(process.execPath, [join(ROOT, "scripts/gen-fixtures.mjs"), tmp], { stdio: "pipe" });
    const committed = [...listFiles("test/fixtures/statements", ".txt"), ...listFiles("test/fixtures/expected", ".json"), "test/fixtures/ledger/household-usuk.json"];
    for (const rel of committed) {
      const sub = rel.replace(/^test\/fixtures\//, "");
      const a = readFileSync(join(ROOT, rel));
      const b = readFileSync(join(tmp, sub));
      assert.ok(a.equals(b), `${rel}: regenerated bytes differ from committed`);
    }
  } finally {
